...
const Comment = () => {
  ...
  const [replyText, setReplyText] = useState(""); // Lưu trữ nội dung phản hồi
  const [replyingTo, setReplyingTo] = useState(null); // Lưu trữ ID của comment mà bạn đang reply
  ...
  const handleReply = (commentId) => {
    if (replyText.trim()) {
      axios
        .post("http://localhost:9999/replies", {
          text: replyText,
          commentId: commentId,
          userId: userId,
        })
        .then((response) => {
          if (response.status === 201) {
            // Thêm phản hồi vào danh sách comment tương ứng
            ...
            setReplyText(""); // Xóa nội dung sau khi gửi phản hồi
            setReplyingTo(null); // Đóng form reply
          } else {
            console.log("Reply thất bại");
          }
        })
        .catch((error) => {
                ...
                {updateInput && selectedComment === index ? (
                  ...
                  <div>
                    ...
                    {c.replies && c.replies.map((reply, replyIndex) => (
                      <div key={replyIndex} className="reply" style={{ marginLeft: "20px", marginTop: "5px" }}>
                        <p><strong>{reply.userId.fullname}:</strong> {reply.text}</p>
              ...
              {replyingTo === c._id && (
                <Form style={{ display: 'flex', alignItems: 'center', marginTop: '10px' }}>
                  <Form.Control
                    ...
                    className="large-textarea" // Áp dụng class mới để thay đổi kích thước ô nhập phản hồi
